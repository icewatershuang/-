# -*- coding: utf-8 -*-
"""V3.3_760：把修好的 dashboard.html 打进包，并原地升版本号（758/V3.1 -> 760/V3.3）。
...
OUT  = r"D:\workbuddy\2026-09-23-08-21-00\KDashBoardL_V3.3_760.apk"
OUT2 = r"D:\workbuddy\2026-09-23-08-21-00\KDashBoardL_V3.3_760_v123.apk"

NEW_CODE = 760
NEW_NAME = "V3.3"          # 必须与旧的 "V3.1" 等长（原地改写）

UNSIGNED = WORK + r"\u6.apk"
ALIGNED  = WORK + r"\a6.apk"
...
strings, ranges, elems = parse_axml(mf)
vals = {}
for (p, attrStart, attrSize, attrCount, apos) in elems:
    for i in range(attrCount):
        aoff = apos + i*attrSize
        ns, name, raw = struct.unpack_from("<III", mf, aoff)
        idx = name & 0x7fffffff
        nm = strings[idx] if idx < len(strings) else None
        if nm in ("versionCode", "versionName", "minSdkVersion", "package"):
            if nm in ("versionName", "package"):
                vals[nm] = strings[struct.unpack_from("<I", mf, aoff+16)[0]]
            else:
                vals[nm] = struct.unpack_from("<I", mf, aoff+16)[0]
print("manifest:", vals)
